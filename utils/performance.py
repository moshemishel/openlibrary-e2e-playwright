...
Module-level state: every call appends to `_RESULTS`. The
`pytest_sessionfinish` hook in `conftest.py` reads this list at the
end of the run and writes `performance_report.json` once. Keeping
the list inside this module avoids threading a context object through
every call site.
...
# Every measurement, in call order. Snapshots are read by the
# `pytest_sessionfinish` hook to produce performance_report.json.
...
async def measure_page_performance(page: Page, url: str, threshold_ms: int) -> dict[str, Any]:
    ...
    A breach (any non-null metric > threshold_ms) is logged at WARNING
    level. The function still returns the record. Per spec: warning,
    not failure.
    """
    await page.goto(url)
...
def get_results() -> list[dict[str, Any]]:
    """Snapshot of every measurement taken in the current process."""
